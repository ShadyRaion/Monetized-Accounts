...
import { AFFILIATE_TIERS, calculateAffiliateCommissionRate, getTierByRate, normalizeCommissionRate } from "../utils/affiliateTiers.js";
import { broadcastEvent } from "../sse.js";
const getAffiliateTierInfo = (commissionRate) => {
    const normalizedRate = normalizeCommissionRate(commissionRate);
    return getTierByRate(normalizedRate);
};
const getNextTierGoal = (commissionRate, autoUpgradeEnabled) => {
...
const shouldCommissionAutoUpgrade = (commissionRate, purchaseCount, existingRate) => {
...
export async function ensureAffiliateForUser(userId, payoutMethod, data) {
...
export const applyAffiliate = async (req, res) => {
    try {
        const rawUserId = req.user?.id;
        if (!rawUserId)
            return res.status(401).json({ message: 'Authentication required' });
        const userId = String(rawUserId);
        const { platforms, isContentCreator, payoutMethod: requestedPayoutMethod, payoutAddress: requestedPayoutAddress, paymentMethod } = req.body ?? {};
        ...
        if (process.env.NODE_ENV !== 'production') {
            return res.status(500).json({ message: 'Internal server error', error: String(error), stack: error?.stack });
        }
        return res.status(500).json({ message: 'Internal server error' });
    }
};
async function enrichAffiliateWithMetrics(affiliate) {
    ...
    const referredOrders = referredUserIds.length
        ...
        : [];
    const totalProductsByReferrals = referredOrders.reduce((sum, order) => sum + (order.items?.length ?? 0), 0);
    const totalSpentByReferrals = referredOrders.reduce((sum, order) => sum + Number(order.totalAmount ?? 0), 0);
    const purchaseEarnings = affiliate.purchases?.reduce((sum, purchase) => {
        const orderStatus = purchase.orderItem?.order?.status ?? '';
        if (typeof orderStatus !== 'string')
            return sum;
        if (!['completed', 'Completed'].includes(orderStatus))
            return sum;
        return sum + Number(purchase.commissionAmount ?? 0);
    }, 0) ?? 0;
    const pendingEarnings = affiliate.purchases?.reduce((sum, purchase) => {
        if (!purchase || typeof purchase.status !== 'string')
            return sum;
        const orderStatus = purchase.orderItem?.order?.status ?? '';
        if (!['completed', 'Completed'].includes(orderStatus))
            return sum;
...
export const adminListAffiliates = async (req, res) => {
    try {
        ...
        const list = await Promise.all(affiliates.map((affiliate) => enrichAffiliateWithMetrics(affiliate)));
        res.json(list);
    }
    catch (error) {
        console.error(error);
...
function parseCommissionRateInput(value) {
...
function getAffiliateUpdatePayload(body) {
    const payload = {};
...
export const updateMyAffiliate = async (req, res) => {
    try {
        const userId = String(req.user?.id);
        if (!userId)
            return res.status(401).json({ message: 'Authentication required' });
        ...
        if (typeof data.commissionRate === 'number') {
            ...
            console.log('[affiliate updateMyAffiliate] existingRate=', existingRate, 'inputRate=', data.commissionRate, 'purchaseCount=', purchaseCount);
            ...
            if (typeof data.commissionRateAutoUpgradeEnabled !== 'boolean') {
                data.commissionRateAutoUpgradeEnabled = shouldCommissionAutoUpgrade(data.commissionRate, purchaseCount, existingRate);
...
export const adminUpdateAffiliate = async (req, res) => {
    try {
        ...
        if (typeof data.commissionRate === 'number') {
            ...
            data.commissionRate = normalizeCommissionRate(data.commissionRate);
            const existingRate = normalizeCommissionRate(existing?.commissionRate);
            console.log('[affiliate adminUpdateAffiliate] existingRate=', existingRate, 'inputRate=', data.commissionRate, 'purchaseCount=', purchaseCount);
            // Only recompute auto-upgrade flag if not explicitly provided in request
            if (typeof data.commissionRateAutoUpgradeEnabled !== 'boolean') {
                data.commissionRateAutoUpgradeEnabled = shouldCommissionAutoUpgrade(data.commissionRate, purchaseCount, existingRate);
...
export const adminPayAffiliate = async (req, res) => {
    try {
        ...
        if (purchases.length) {
            await prisma.$transaction(purchases.map((p) => prisma.affiliatePurchase.update({ where: { id: p.id }, data: { status: 'Paid' } })));
        }
