...
export const previewProducts = async (req, res) => {
    try {
...
export const listProducts = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Internal server error', error: String(error?.message || error) });
    }
};
...
function parseFollowersValue(value) {
    ...
    if (typeof value === "string") {
        ...
        if (!Number.isFinite(numberPortion))
            return 0;
        if (suffix === "K")
            return Math.round(numberPortion * 1000);
        if (suffix === "M")
            return Math.round(numberPortion * 1000000);
...
function buildProductData(body) {
    const { platform, type, followers, hasVerificationFee, verificationPrice, price, description, features, badge, transferTime, inStock } = body;
    const data = {};
    ...
    if (followers !== undefined) {
        ...
        if (!Number.isFinite(parsed))
            console.warn('buildProductData: followers parsed to non-finite', followers, parsed);
    }
    ...
    if (features !== undefined)
        data.features = normalizeFeatures(features);
    if (badge !== undefined)
...
export const deleteProduct = async (req, res) => {
    try {
        ...
        const isPrismaConstraintError = typeof error === 'object' &&
            error !== null &&
            'code' in error &&
            error.code === 'P2003';
        if (errorMessage.includes('OrderItem_productId_fkey') ||
