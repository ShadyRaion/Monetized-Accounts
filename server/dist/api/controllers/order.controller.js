...
import { normalizeCommissionRate, calculateCommissionAmount, getNextTierRate } from "../utils/affiliateTiers.js";
const isAffiliateApproved = (status) => {
    return ['accepted', 'active'].includes(String(status || '').trim().toLowerCase());
};
const resolveReferralCode = async (userId, requestReferralCode) => {
...
export const createOrder = async (req, res) => {
    try {
        ...
        const products = (await prisma.product.findMany({ where: { id: { in: itemProductIds } } }));
        ...
        try {
            broadcastEvent({ type: 'order', action: 'created', data: order });
        }
        catch (e) { }
...
export const calculateOrderItemCommissionAmount = (item, rate) => {
...
const processAffiliateOrderCompletion = async (order) => {
    if (!order || !order.referralCode)
        return;
    ...
    if (!affiliate || !isAffiliateApproved(affiliate.status))
        return;
    console.log('[affiliate order completion] orderId=', order.id, 'referralCode=', order.referralCode, 'affiliateUserId=', affiliate.userId, 'commissionRate=', affiliate.commissionRate, 'commissionRateAutoUpgradeEnabled=', affiliate.commissionRateAutoUpgradeEnabled, 'affiliateStatus=', affiliate.status);
    ...
    console.log('[affiliate order completion] autoUpgradeEnabled=', autoUpgradeEnabled, 'currentRate=', currentRate, 'currentRateRaw=', currentRateRaw, 'purchaseCount=', prevCount);
...
const formatReviewDisplayName = (name) => {
    if (!name)
        return 'Anonymous';
    ...
    if (parts.length === 0)
        return 'Anonymous';
    if (parts.length === 1)
        return maskAnonymousName(parts[0]);
    return maskAnonymousName(name);
};
const createAutoReviewForOrder = async (order) => {
    if (!order || order.review || order.status !== 'Completed')
        return null;
...
export const updateOrderStatus = async (req, res) => {
    try {
        ...
        const data = { status: normalizedStatus, statusManualOverride: true };
        ...
        try {
            broadcastEvent({ type: 'order', action: 'updated', data: refreshedOrder });
        }
        catch (e) { }
...
export const adminUpdateOrderDelivery = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const allDelivered = updatedOrder.items.every((item) => Boolean(item.accountDetails));
        ...
        try {
            broadcastEvent({ type: 'order', action: 'updated', data: finalOrder });
        }
        catch (e) { }
        res.json(finalOrder);
    }
