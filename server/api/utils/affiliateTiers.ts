 ...
 */
export const normalizeCommissionRate = (rate: any): number => {
 ...
 */
export const calculateAffiliateCommissionRate = (previousPurchases: number): number => {
  if (previousPurchases >= 100) return 40;
  if (previousPurchases >= 50) return 35;
  if (previousPurchases >= 25) return 30;
  if (previousPurchases >= 10) return 25;
  return 20;
};
 ...
 */
export const getNextTierRate = (currentRate: number, totalPurchases: number): number => {
 ...
 */
export const getTierByRate = (rate: number): (typeof AFFILIATE_TIERS)[number] => {
  const normalizedRate = normalizeCommissionRate(rate);
  if (!Number.isFinite(normalizedRate)) return AFFILIATE_TIERS[0];

  let currentTier: (typeof AFFILIATE_TIERS)[number] = AFFILIATE_TIERS[0];
  for (const tier of AFFILIATE_TIERS) {
