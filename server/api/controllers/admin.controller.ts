...
export const listCustomers = async (req: any, res: Response) => {
  try {
    const users = await prisma.user.findMany({
    ...
    const mapped = users.map((u: any) => {
      const userOrders = (u.orders || []) as CustomerOrder[];
      const totalSpent = userOrders.reduce((s: number, o: CustomerOrder) => s + (o.totalAmount || 0), 0);
      const ordersCount = userOrders.length;
      const firstPurchaseDate = ordersCount > 0 ? new Date(Math.min(...userOrders.map((o: CustomerOrder) => new Date(o.createdAt).getTime()))).toISOString().split('T')[0] : '';
      const lastPurchaseDate = ordersCount > 0 ? new Date(Math.max(...userOrders.map((o: CustomerOrder) => new Date(o.createdAt).getTime()))).toISOString().split('T')[0] : '';
...
export const autoCompleteOrders = async (req: any, res: Response) => {
  try {
...
export const listBlacklist = async (req: any, res: Response) => {
  try {
...
export const addToBlacklist = async (req: any, res: Response) => {
  try {
    const { email, reason } = req.body
    if (!email) return res.status(400).json({ message: 'Email required' })
    const item = await prisma.blacklist.create({ data: { email: email.toLowerCase(), reason } })
    const user = await prisma.user.findUnique({ where: { email: email.toLowerCase() } })
    if (user) await prisma.user.update({ where: { id: user.id }, data: { isBanned: true } })
...
export const removeFromBlacklist = async (req: any, res: Response) => {
  try {
    const { id } = req.params
