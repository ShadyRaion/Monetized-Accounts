...
function AffiliatePageContent() {
  ...
  const [scrollTarget, setScrollTarget] = useState<"pending" | "payment" | null>(null)
  ...
  useEffect(() => {
    ...
    } else if (searchParams.get("submitted") === "true") {
      setScrollTarget("pending")
    }
  }, [searchParams])

  useEffect(() => {
    const shouldScrollToPending = Boolean(
      pendingSectionRef.current &&
      !hasScrolledToPending &&
      (
  ...
  }, [affiliate?.status, submitted, hasScrolledToPending, scrollTarget])

  useEffect(() => {
    if (!paymentSectionRef.current || hasScrolledToPayment || scrollTarget !== "payment") {
      return
    }

    paymentSectionRef.current.scrollIntoView({ behavior: "smooth", block: "start" })
    setHasScrolledToPayment(true)
  }, [scrollTarget, hasScrolledToPayment])
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main>
        ...
        <section id="pending" className="py-20" ref={pendingSectionRef}>
          <div className="container mx-auto px-4">
            <div className="max-w-2xl mx-auto">
              {affiliate?.status === "pending" ? (
