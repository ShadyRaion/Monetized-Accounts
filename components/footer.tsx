...
export function Footer() {
  ...
  const [panelStatus, setPanelStatus] = useState<"idle" | "success" | "hidden">("idle")
  ...
  const { stats, addSubscriber, subscribers, isLoaded } = useStoreData()
  ...
  const isGuestSubscribed = !isAuthenticated && guestSubscribed
  const isSubscribed = Boolean(isUserSubscribed || isGuestSubscribed)
  const shouldRenderNotifyBlock = panelReady && panelStatus !== "hidden" && (isSubmittingSubscriber || !isSubscribed || panelStatus === "success") && (!isAuthenticated || isLoaded || !user)
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      setGuestSubscribed(!!window.localStorage.getItem('newsletter_subscribed'))
      setPanelReady(true)
    }
  }, [])

  useEffect(() => {
    if (isSubmittingSubscriber || panelStatus === "success") {
      return
    }
    ...
    if (isUserSubscribed) {
      setPanelStatus("hidden")
      setIsFadingOut(false)
    } else if (!isGuestSubscribed) {
      setPanelStatus("idle")
      setIsFadingOut(false)
    }
  }, [isUserSubscribed, isGuestSubscribed, panelStatus, isSubmittingSubscriber])

  useEffect(() => {
    if (typeof window !== 'undefined' && isAuthenticated && !isUserSubscribed && window.localStorage.getItem('newsletter_subscribed')) {
      window.localStorage.removeItem('newsletter_subscribed')
      setGuestSubscribed(false)
    }
  }, [isAuthenticated, isUserSubscribed])
  ...
  return (
    <footer className="bg-black text-white">
      {shouldRenderNotifyBlock ? (
        <div className={`border-b border-white/10 transition-opacity duration-300 ease-out ${isFadingOut ? 'opacity-0' : 'opacity-100'}`}>
          <div className="container mx-auto px-4 py-4 sm:py-12">
            <div className="max-w-2xl mx-auto text-center">
              ...
              <p className="text-gray-400 text-[11px] sm:text-sm md:text-base mb-3 sm:mb-6">
                Subscribe to get notified about restocking and our latest offers.
              </p>
              <div className="min-h-[52px] flex items-center justify-center">
                {panelStatus === "success" ? (
                  <div className="space-y-2">
                    <p className="text-[#25F4EE] font-medium text-[10px] sm:text-sm">Thank you! Check your email.</p>
                    <p className="text-gray-400 text-[10px] sm:text-sm">Please check your inbox for confirmation details.</p>
                  </div>
                ) : (
